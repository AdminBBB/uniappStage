...
export function SuperViewOperate (props) {
    const { state: { operateType, currentResourceSupply }, dispatch } = useContext(COMMON_CONTEXT);
    ...
    const superReviewInfo = REVIEW_RESULT_MAP_BUILD(superViewOperateReviewMap?.reviewResult, ([31].includes(operateType) && checkRight()), superViewOperateReviewMap?.reviewComments, true);
