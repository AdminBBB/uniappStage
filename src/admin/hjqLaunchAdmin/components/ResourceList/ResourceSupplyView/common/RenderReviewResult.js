...
const itemTypeAdmin = [2, 3];
export function RenderReviewResult (props) {
    const { state: { operateType, currentResourceSupply, currentLauncherData: { itemType } }, dispatch } = useContext(COMMON_CONTEXT);
    const { checkRight } = useCheckAuthRight();
    const { ReviewBo, type } = props;
    const lcrMngOperaters = useLcrMngOperaters();
    const isReview = [31].includes(operateType) && ((checkRight([1]) && type === 'img') || (checkRight([itemTypeAdmin[itemType]]) && type === 'txt'));
