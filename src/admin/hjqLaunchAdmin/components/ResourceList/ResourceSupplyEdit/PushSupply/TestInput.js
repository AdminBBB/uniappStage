    ...
    return <Modal
        ...
        <Form {...FORM_LAYOUT} form={formEditTestRef} name="formEditTestRef">
            {testInput.map((pro, index) => {
                const rules = [{ pattern: regex.reg.cellphone, message: '请输入正确手机号' }];
                if (index === 0) {
                    rules.push({ required: true, message: '请输入正确手机号' });
                }
                return <Item fieldKey={index} key={index} name={pro.name} label="手机号" rules={rules}>
