...
export function ResourceBasicInfo (props) {
    const { state: { LcrDependencesData: { patternList }, currentLauncherData, operateType } } = useContext(COMMON_CONTEXT);
    ...
    const $Operaters = (resource) => {
        ...
        const editAuthority = [0, 1, 10, 11, 12, 13, 22].includes(operateType) && !([6].includes(currentLauncherData.itemStatus));
