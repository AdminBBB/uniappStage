...
export function OperateDataTable () {
    ...
    const { operateTimes, authType, localauthType } = state;
    ...
    const columns = [
        ...
        {
            title: <div className={'theade-operate'}><span>操作</span> <Button type={'primary'} onClick={addOperateTime}><PlusOutlined />新建排期</Button></div>,
            dataIndex: 'id',
            aign: 'center',
            render (id, operateTime) {
                const { opTimeStatus } = operateTime;
                const couldView = localauthType !== 1;
                const couldEdit = ([0, 3].includes(localauthType) && opTimeStatus === 1) || (localauthType === 0 && opTimeStatus === 2);
                const couldDelete = opTimeStatus === 1 && localauthType === 0;
                const couldViewDate = opTimeStatus === 3;
                const couldReview = localauthType === 0;
