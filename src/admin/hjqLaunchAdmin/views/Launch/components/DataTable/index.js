        ...
        },
        {
            ...
            render: (itemId, record) => {
                ...
                const couldEditStyle = (
                    // 超管、产品2/运营3管理、产品4/运营5申请 只有在条目保存未提交，追着追加条目保存未提交的情况下才可以编辑
                    (itemStatus !== 6 && extraItemStatus !== 6 && ((checkRight([], itemType, createBy) && (itemStatus === 1 || extraItemStatus === 1)))) ||
                    // 超管、产品2/运营3管理 在2已提交、5已生效待上线、7已生效已下线 情况下可以编辑
                    (checkRight([], itemType) && ([2, 5, 7].includes(itemStatus) || [2, 5, 7].includes(extraItemStatus)))
                ) ? 'nor' : 'noactive';
                const couldAdtStyle = (checkRight([], itemType, createBy) && [4, 5, 6, 7, 8].includes(itemStatus) && extraItemStatus === 0) ? 'nor' : 'noactive'; // extraItemStatus === 0 没有追加
                ...
                return <div className="c-status">
                    <a className={couldViewStyle} onClick={() => dispatch({ viewLauncherInModalByItemId: itemId, operateType: 20 })}>查看</a>
                    <a className={couldEditStyle} onClick={() => navigate(`/editLauncher/${itemId}/10`)}>编辑</a>
                    <a className={couldCopyStyle} onClick={() => navigate(`/editLauncher/${itemId}/11`)}>复制</a>
                    <a className={couldDeltStyle} onClick={() => dispatch({ deleteModalItemId: itemId })}>删除</a>
